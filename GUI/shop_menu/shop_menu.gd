...
const MENU_FOCUS = preload("res://title screen/audio/menu_focus.wav")
const MENU_SELECT = preload("res://title screen/audio/menu_select.wav")
...
func purchase_item( item : ItemData ) -> void:
	...
	if can_purchase:
		...
		play_audio( ERROR )
		gems_animation_player.play( "broke" )
